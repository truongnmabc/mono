...
async function setupSingleApp(appShortName) {
  ...
  try {
    ...
    s.start('Bước 3: Xử lý dữ liệu topics và tests...');
    ...
    s.start('Bước 2: Lưu data seo home');

    const dataHome = {
      // seo: seo,
      // topics: listT.map((item) => ({
      //   id: item.id,
      //   icon: item.icon,
      //   name: item.name,
      //   slug: item.slug,
      ...
      tests: {
        finalTests: {
          id: 'FT',
          testId: topicsAndTest.tests.finalTests[0].id,
        },
        practiceTests: {
          id: 'PT',

          listPracticeTest: topicsAndTest.tests?.practiceTests.map((item) => ({
            id: item.id,
          })),
        },
        diagnosticTest: {
        ...
        branchTest: {
          id: 'BT',
          listBranchTest: branchTest.map((item) => ({
            id: item.id,
            slug: item.slug,
          })),
        },
    ...
    saveJSONFile(
      path.join(DATA_PATH, '/apps/single/src/data/home/index.json'),
      dataHome
    );
    s.stop('Bước 2: Xong');
