...
const GridLeftCustomTest = () => {
  ...
  useEffect(() => {
    ...
    if (listQuestion?.length === 0 && isDataLoaded) setOpenModalSetting(true);
  ...
  const handleDelete = useCallback(async () => {
    if (itemSelect) {
      ...
      setListTest((prev) => {
        const updatedList = prev.filter((item) => item.id !== itemSelect.id);
        if (!updatedList.length) {
          dispatch(resetState());
          setItemSelect(null);
        } else if (startTest) {
          const index = updatedList.findIndex((i) => i.id === startTest.id);
              ...
              {listTest?.map((item, index) => (
                <div
                  ...
                  className={clsx(
                    ...
                    {
                      'border-primary': indexSubTopic - 1 === index,
                    }
                  )}
                >
                  <Tooltip
                  ...
                  >
                    <p
                      className={clsx(
                        ...
                        {
                          'pointer-events-none': indexSubTopic - 1 === index,
                        }
                      )}
                      onClick={() => {
