...
export const clearDbLocalBeforeSync = async ({
  isClearDianosticTest,
}: {
  isClearDianosticTest: boolean;
}) => {
  try {
    // Xóa dữ liệu cũ
    if (db) {
    ...
    const filteredTests = isClearDianosticTest
      ? testsResponse.data.filter(
