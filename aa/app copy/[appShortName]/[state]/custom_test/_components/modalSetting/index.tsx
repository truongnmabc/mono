...
const ModalSettingCustomTest: React.FC<{
  ...
  useEffect(() => {
    if (item && listTopic.length > 0) {
      setValue(
        'selectListTopic',
        listTopic.filter((topic) => item.topicIds?.includes(topic.id)) || []
      );
    }
  ...
  const onSubmit = async (data: IFormState) => {
    try {
      ...
      const remainderQuestionTopic = data.count % data.selectListTopic.length;
      ...
      if (!isUpdate || isDisabled) {
        const listIds = item?.groupExamData.flatMap((i) => i.questionIds) || [];
        ...
        dispatch(
          startCustomTest({
            ...
            passingThreshold: data.passing,
            gameDifficultyLevel: data.selectFeedback,
            currentSubTopicIndex: isUpdate ? currentIndex : indexSubTopic,
          })
        );
      }
      dispatch(shouldLoading());
